...
//     // Extract push tokens from users
//     const pushTokens = users.map(user => user.push_token).filter(Boolean);

//     // Check if there are any users with push tokens
...
//       await Notifications.scheduleNotificationAsync({
//         content: {
//           title: 'Your Alert Title', // Customize the title
//           body: selectedMessage.name, // Use the selected message as the body
//           sound: 'default',
//         },
//         to: pushToken,
//       });
...
//     console.log('Alerts sent to all users.');
//     Alert.alert('התראה נשלחה בהצלחה');
//   } catch (error) {
...
//     // Ensure that the response data has a 'data' property and it is an array
//     if (!responseData.data || !Array.isArray(responseData.data)) {
//       throw new Error('Invalid response: data is not an array');
//     }
...
// };
  ...
  //console.log("selectedMessage2:", selectedMessage);
